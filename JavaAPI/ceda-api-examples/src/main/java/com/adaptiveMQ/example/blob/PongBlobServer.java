...
public class PongBlobServer extends Thread
{
    private static final InternalLogger logger = InternalLoggerFactory.getInstance(PongBlobServer.class);
    ...
    {
        PongBlobServer pongServer = new PongBlobServer();
                            ...
                            if (imsg.getMessageType() == IMessage.MESSAGE_TYPE_DATA) {
                                ...
                                try {
                                    ...
                                    logger.info("on Message, topic:" + msg.getDestination().getName()
                                            + ", data:" + msg.getMessageBody().getString((short) 5)
                                            + ", elapsed:" + diff + "(ms)");

                                    String blobstr = new String(msg.getMessageBody().getBlobField(), java.nio.charset.StandardCharsets.UTF_8);
                                    String[] lines = blobstr.split("\n");
                                    for(String line : lines) {
                                        logger.info(line);
                                    }
                                    Message rmsg = msg.createReplyMessage();
                                    rmsg.setMessageBody(msg.getMessageBody());
